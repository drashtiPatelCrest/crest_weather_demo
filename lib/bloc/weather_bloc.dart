...
class WeatherBloc extends HydratedBloc<WeatherEvent, WeatherState> {
  ...
  Future<void> _onGetLocationInfoEvent(Emitter<WeatherState> emit) async {
    ...
    if (position != null) {
      emit(state.copyWith(
          status: WeatherStateStatus.location, position: position));
    } else {
    ...
    try {
      ...
      if (event.position != null) {
        WeatherModel? weatherModel =
            await _apiRepository.fetchWeatherInfo(position: event.position!);
        (weatherModel != null)
            ? emit(state.copyWith(
                weatherModel: weatherModel,
            ...
            : emit(state.copyWith(
                status: WeatherStateStatus.failed, weatherModel: null));
      } else {
        emit(state.copyWith(
  ...
  Map<String, dynamic>? toJson(WeatherState state) {
    if (state.status == WeatherStateStatus.loaded) {
      return serializers.serializeWith(
          WeatherModel.serializer, state.weatherModel) as Map<String, dynamic>;
